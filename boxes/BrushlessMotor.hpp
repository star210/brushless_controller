...
public:
  ...
  uint32_t setSpeed(float rpm) {
    mDriveDelay = speedToDelay(rpm);
  }

  uint32_t speedToDelay(float rpm) {
    return static_cast<uint32_t>(15238.09 / rpm * 60); 
  }
  ...
  void turn() {
    ...
    enable();
    mStopped = false;
    mMinDelay = speedToDelay(MAX_SPEED);
  }

  void setDistance(float degrees) {
    mDriveSteps = static_cast<uint32_t>(7 * degrees);
  }

  void update(uint32_t currentMicros) {
    if (mStepCounter < mDriveSteps) {
      ...
      if (currentMicros - mLastDrive > mDriveDelay) {
        ...
        if (mStepCounter > mDriveSteps - ACCELERATION_DISTANCE) {
          mDriveDelay+= ACCELERATION;
        }
        else if (mDriveDelay > mMinDelay) {
          mDriveDelay-= ACCELERATION;
        }

        mDriveDelay = max(mDriveDelay, mMinDelay);
//        
//        else if (mStepCounter < ACCELERATION_DISTANCE) {
//          mDriveDelay-= ACCELERATION;
//          //Serial.println("acc");
//        }
        mStepCounter++;
        //Serial.println(mDriveDelay);
        if (mStepCounter == mDriveSteps) {
          mStopped = true;
        }
      }
  ...
  void updateBasic(uint32_t currentMicros) {
    if (currentMicros - mLastDrive > mDriveDelay) {
        drive();
        mLastDrive = currentMicros;
    }
...
protected:
  ...
  uint32_t mStepCounter, mDriveSteps;
...
BrushlessMotorDriver::BrushlessMotorDriver(uint8_t p1, uint8_t p2, uint8_t p3, uint8_t enable):
  BrushlessMotor(p1, p2, p3, enable), mLastDrive(0), mStopped(true) {
